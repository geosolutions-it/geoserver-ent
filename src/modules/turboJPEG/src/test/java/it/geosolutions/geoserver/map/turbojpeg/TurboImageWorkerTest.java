...
public class TurboImageWorkerTest extends Assert {
    ...
    public void errors() throws IOException{
        ...
        try{
        	new TurboJpegImageWorker(input).writeTurboJPEG(new FileOutputStream(output), 1.5f);
        ...
        try{
        	new TurboJpegImageWorker(input).writeTurboJPEG(new FileOutputStream(output), -.5f);
        	assertFalse("We should not be allowed to specify compression ratios > 1",true);
        } catch (Exception e) {
			// TODO: handle exception
		}
